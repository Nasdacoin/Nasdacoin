...
unsigned int nStakeMinAge = 15 * 24 * nOneHour; // 15 days as zero time weight
...
const string strMessageMagic = "Nasdacoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nTime, bool bCoinYearOnly)
{
	...
	if (nTime > 1530316800) // Human time (GMT): Saturday, June 30, 2018 12:00:00 AM
		nRewardCoinYear = 10 * CENT;
	if (nTime > 1546214400) // Human time (GMT): Monday, December 31, 2018 12:00:00 AM
		nRewardCoinYear = 8 * CENT;
	if (nTime > 1561852800) // Human time (GMT): Sunday, June 30, 2019 12:00:00 AM
		nRewardCoinYear = 7 * CENT;
	if (nTime > 1577750400) // Human time (GMT): Tuesday, December 31, 2019 12:00:00 AM
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("Nasdacoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Nasdacoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Nasdacoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Nasdacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "nasdacoin genesis";
        CTransaction txNew;
        txNew.nTime = 1513893241;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1513893241;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 826586 : 992005265;
...
unsigned char pchMessageStart[4] = { 0xb4, 0xf4, 0xe2, 0xe5 };
