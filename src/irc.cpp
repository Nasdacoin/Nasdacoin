...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Nasdacoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NasdacoinTEST2\r");
            Send(hSocket, "WHO #NasdacoinTEST2\r");
        } else {
            // randomly join #Nasdacoin00-#Nasdacoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Nasdacoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Nasdacoin%02d\r", channel_number).c_str());
        }
