Nasdacoin-qt: Qt4 GUI for Nasdacoin
===============================
...
Alternatively, install Qt Creator and open the `Nasdacoin-qt.pro` file.

An executable named `Nasdacoin-qt` will be built.
...
A warning for people using the *static binary* version of Nasdacoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Nasdacoin is linked against libdb5.3.
...
installed causes Nasdacoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
